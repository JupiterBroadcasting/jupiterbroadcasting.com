...
def test_mobile_live_indicator(
    ...
    navbar: Locator = mobile_device.locator("#mainnavigation.is-live").locator('.navbar-burger')
    # wait for navbar to be visible
    expect(navbar).to_be_visible()
    # check if live indicator is red
    assert navbar.evaluate(
    ...
    expect(mobile_device.locator("#mainnavigation.is-live").locator("#livebutton"),).to_have_css(
        name="background-color",
        value="rgb(255, 0, 0)",  # red
    )
